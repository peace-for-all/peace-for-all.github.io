...
The default home page is a small living diary: a visitor or compatible browser agent can open a supplied engineering scene, follow a clue, connect fixed published evidence, and shape an editable local-only hello. Agent inferences are visibly labelled as provisional, while the underlying portfolio remains available at `/?view=all` and in the same semantic HTML. The review-response case file adds a shared investigation of an ambiguous marketplace message. All WebMCP tools reuse public client-side data and visible UI behavior; no model, account, API key, or remote MCP server is part of the site. With JavaScript disabled, the full conventional portfolio is shown.
