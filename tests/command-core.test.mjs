...
test("aliases normalize",()=>{assert.equal(parse(" help ").command,"help");assert.equal(parse("man   valia").command,"help");assert.equal(parse("whoami").command,"about");assert.equal(parse("ls").command,"work");assert.equal(parse("cat coast.jpg").command,"open coast.jpg");});
